...
Gaussian Mixture Modeling (GMM) is another form of unsupervised classification and falls into a category of algorithms referred to as *model-based clustering*, where a distributional form is assumed for individual clusters.  Let's rewind a bit, and just review what I am referring to by the terms *clustering* and *cluster*.  Clustering is simply grouping similar data points together based on their attributes, where *similarity* is typically quantified using a distance calculation, such as Euclidean distance.  A *cluster* is just a group resulting from a clustering algorithm.
...
In GMM, it is assumed that there are a certain number of Gaussian distributions, each of which represents a cluster. This is a fairly intuitive assumption, since we are all familiar with the bell-shaped Gaussian or *Normal* distribution. The process is iterative, estimating the parameters for each cluster's distribution, and then computing the probability of each observation belonging to each cluster.  These steps are repeated until the solution stops changing significantly between iterations. Unlike k-means, GMM allows for elliptical decision boundaries, which may be a better choice for some data than circular boundaries.  An excellent discussion of the differences between k-means and GMM can be found in this post on *Analytics Vidhya*:
