...
```
    ...
    <seaborn.axisgrid.PairGrid at 0x2c0f288e188>
...
### Scaling the input curves

Note that above we didn't do any scaling of the input variables before clustering.  Let's scale them and repeat.  Let's drop the columns we don't need first.


```python
df2_toscale = df2.drop(['Well Name', 'Depth', 'Facies', 'Facies_names', 'gmm_labels'], axis=1)
df2_toscale.head()
```
...
Now we can import the *preprocessing* subpackage from *sklearn* that contains the *StandardScaler* function.


```python
from sklearn import preprocessing
scaler = preprocessing.StandardScaler().fit(df2_toscale)
df2_scaled = scaler.transform(df2_toscale)
```
...
ax = fig.gca()
pd.DataFrame(df2_scaled).hist(ax=ax)
plt.show();
```
...
![png](index_files/figure_html/gmm_modeling_23_1.png)
    


Now regenerate the GMM clusters from the scaled data.


```python
# create the model object and specify number of clusters
gmm2 = GaussianMixture(n_components=9)

# fit model to data set
gmm2.fit(df2_scaled)

# predict the cluster labels
gmm_labels = gmm.predict(df2_scaled)

# add the labels back to the dataframe as a new column
#df2_scaled['gmm_labels'] = gmm_labels
```

And add the labels back into the original dataframe.
...
And replot the logs along with the facies and clusters for the *SHRIMPLIN* well.
...
![png](index_files/figure_html/gmm_modeling_30_0.png)
...
```
    ...
    <seaborn.axisgrid.PairGrid at 0x2c0f9122d08>




    
![png](index_files/figure_html/gmm_modeling_31_1.png)
